...
```bash
ReactApp$ sam build --use-container
```
...
```bash
ReactApp$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ReactApp$ sam local start-api
ReactApp$ curl http://localhost:3000/
```
...
```bash
ReactApp$ sam logs -n HelloWorldFunction --stack-name "reactapp" --tail
```
...
```bash
ReactApp$ pip install -r tests/requirements.txt --user
# unit test
ReactApp$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
ReactApp$ AWS_SAM_STACK_NAME="reactapp" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "reactapp"
```
